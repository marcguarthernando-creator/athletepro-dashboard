...
const Sidebar: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    // Initial check in case it was updated in this session
    handleStorageChange();
    return () => window.removeEventListener('storage', handleStorageChange);
  }, []);

  return (
    <aside className="w-64 flex-none border-r border-surface-border bg-background-dark hidden md:flex flex-col justify-between p-4 h-full">
      ...
      <div className="flex items-center gap-3 px-1 py-4 mt-auto border-t border-surface-border/30">
        ...
        >
          <div className="size-9 min-w-9 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden border border-primary/20 group-hover/info:border-primary/50 transition-colors">
